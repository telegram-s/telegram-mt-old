 ...
 * Created: 21.07.13 16:12
 */
public class PrimeUtils {

    private static final String TAG = "PrimeFinder";

    private static long findSmallMultiplierSlow(long pq) throws Exception {
        long x = (long) (Math.ceil(Math.sqrt(pq)) + 1);
        int iterations = 0;

        while (true) {
            long y2 = x * x - pq;
            boolean isPerfect = isPerfectSquare(y2);
            if (isPerfect) {
    ...
    private static long safeDiv(long a, long b) {
        ...
        long mathRes = (long) Math.floor(((double) a) / b);
        if (mathRes != res) {
            Logger.d(TAG, "incorrect floor");
        }
        return res;
    }

    private static long findSmallMultiplierShanks(long N) throws Exception {
        if (N % 4 == 1) {
            Logger.w(TAG, "Shanks: N mod 4 ==1");
        }
        ...
        while (true) {
            ...
            while (!isPerfectSquare(q2)) {
                ...
                iterationCount++;
                // i==1
                long b = safeDiv(sqN + p0, q1);
                p1 = b * q1 - p0;
                q2 = q0 + b * (p0 - p1);
            }


            long d = (long) Math.sqrt(q2);
            Logger.d(TAG, "Shanks (" + k + ")" + count + "/" + iterationCount + ", " + d);
            int fistCount = iterationCount;
            long b0 = safeDiv(sqN - p1, d);
            long nP0 = b0 * d + p1;
            long nQ0 = d;
            long nQ1 = (kN - nP0 * nP0) / nQ0;
            ...
            while (p0 != p1) {
                ...
                if (iterationCount > fistCount * 5) {
                    p1 = N;
                    Logger.w(TAG, "Breaking long cycle");
                    break;
                }
            ...
            long res = new BigInteger("" + N).gcd(new BigInteger(p1 + "")).longValue();

            if (res != 1 && res != N) {
                Logger.d(TAG, "Shank: count: " + count);
                long q = N / res;
                if (res < q) {
                    return res;
                } else {
                ...
                if (k > 5) {
                    Logger.w(TAG, "Switching to slow implementation");
                    return findSmallMultiplierSlow(N);
                }
            }
        }
    }
    ...
    public static BigInteger findSmallMultiplier(BigInteger pq) throws Exception {
        long pqL = pq.longValue();
        Logger.d(TAG, "findSmallMultiplier: " + pq);
        long res = findSmallMultiplierLopatin(pqL);
        Logger.d(TAG, "findSmallMultiplier_end: " + res);
